...
app.get('/rotaVer', async(req, res)=>{
    try{
        const [rows] = await pool.query('SELECT nomeModelo FROM carros');
        res.json(rows)
    }
    catch(e){
        console.log("Erro", e)
...
app.post('/testeAdd', async (req, res) => {
    try {
        const { nome } = req.body;
        await pool.query('INSERT INTO carros (nomeModelo) VALUES (?)', [nome]);
...
app.put('/rotaUpdate/:id', async (req, res) => {
    try {
        const { id } = req.params;
        const { nome } = req.body;
        const [result] = await pool.query('UPDATE carros SET nomeModelo = ? WHERE id = ?', [nome, id]);
